/*/AVANCE 2: Creación de funciones (se deja comentado como evidencia de avance 2, ya que si dió el resultado)

// Función cargarCabecero
const cargarCabecero = () => {
    let presupuesto = totalIngresos(ingresos) - totalEgresos(egresos);
    let porcentajeEgreso = (totalEgresos(egresos) / totalIngresos(ingresos)) * 100;

    console.log("Presupuesto: ", formatoMoneda(presupuesto));
    console.log("Porcentaje de Egreso: ", formatoPorcentaje(porcentajeEgreso/100)); //Se divide entre cien porque me marcaba cantidad erronea
    console.log("Total Ingresos: ", formatoMoneda(totalIngresos(ingresos)));
    console.log("Total Egresos: ", formatoMoneda(totalEgresos(egresos)));
};
...
/*Eliminados para hacer prueba de avance 3
...
// Llamando a la función cargarCabecero
cargarCabecero();*/
...
//Define valor de porcentaje hasta dos dígitos
...
//Contenido dinámico del div lista-ingresos
...
const crearEgresoHTML = (egreso) => {
 ...
 //se manda a llamar a las funciones desde la función CargarApp
const cargarApp = () => {
    cargarCabecero();
    cargarIngresos();
    cargarEgresos();
...
const eliminaIngreso = (id) => {
 ...
 //Se declara función agregarDato
const agregarDato = () => {
    ...
    if (tipo && descripcion && valor && valor > 0) {
        ...
        alert("Captura la información completa");
    }

};

//Este apartado hace accesible estas funciones desde el ámbito global 
//para interactuar con ellas desde otros scripts o la consola del navegador.
